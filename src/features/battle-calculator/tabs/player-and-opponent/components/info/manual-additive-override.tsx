import type { AdditiveManualOverride, StatType, TroopType } from '@/domain/battle/calculations';
...
interface ManualAdditiveOverrideProps {
  overrides?: AdditiveManualOverride;
  onChange: (next: AdditiveManualOverride | undefined) => void;
}
...
export default function ManualAdditiveOverride({
  overrides,
  onChange
}: ManualAdditiveOverrideProps) {
  // --------------------------------------------------------------------------
  ...
  const updateOverride = (troop: TroopType, stat: StatType, value: string) => {
    ...
    const next: AdditiveManualOverride = { ...(overrides || {}) };
    ...
    <div className="rounded-lg border border-slate-700/70 bg-slate-900/40 p-4 [data-theme='light']:border-gray-200 [data-theme='light']:bg-white">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-1">
          <div className="text-sm font-semibold text-slate-100 [data-theme='light']:text-gray-900">
            Manual Additive Override (optional)
          </div>
          <p className="text-xs text-slate-400 [data-theme='light']:text-gray-600">
            Enter total additive % per troop and stat. Leave blank to use calculated summary.
          </p>
        </div>
      ...
      <div className="mt-4 space-y-4">
        {TROOP_TYPES.map((troop) => (
          <div
            key={troop}
            className="space-y-2 rounded-md border border-slate-800/60 bg-slate-900/60 p-3 [data-theme='light']:border-gray-200 [data-theme='light']:bg-gray-50"
          >
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {STAT_TYPES.map((stat) => (
                <label
                  key={stat}
                  className="flex flex-col gap-1 text-xs font-medium text-slate-300 [data-theme='light']:text-gray-700"
                >

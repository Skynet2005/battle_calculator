...
export interface DamageModifier {
  ...
  /**
   * Explicit modifier category for stacking purposes.
   * When set, used instead of string-matching heuristic in categorizeModifierType.
   * E.g. "damage", "damageReduction", "attack", "defense", etc.
   */
...
export interface SkillDefinition {
  ...
  icon?: string;
  /** Chance to double normal damage when this skill deals damage (0..1, skill-specific) */
...
/**
 * An active shield that absorbs damage before troops are killed.
 */
export interface ActiveShield {
  /** Source skill or ability that created this shield */
  source: string;
  /** Remaining absorption capacity (in kill-equivalents) */
  remaining: number;
  /** Which troops this shield protects */
  appliesTo: BonusTarget;
}

/**
 * An active stun/control effect preventing actions.
 */
export interface ActiveStun {
  /** Source skill that applied the stun */
  source: string;
  /** Which troop type is stunned */
  target: BonusTarget;
  /** Turns remaining */
  remaining: number;
}
...
export interface BattleConfig {
  ...
  simulations?: number;
  /**
   * Damage formula variant.
   * - "v1": (ATK * LETH) / (DEF * HP) -- full four-stat formula (default)
   * - "v2": (ATK * LETH) / DEF -- HP excluded from denominator (heuristic)
   */
  damageFormulaVersion?: "v1" | "v2";
  /**
   * Matchup multipliers by attacker -> defender type.
   * Used for rock-paper-scissors type advantages.
   */
  matchupMultipliers?: Record<TroopType, Record<TroopType, number>>;
  /**
   * Whether to use row-based formations (front/back).
   * When enabled, attackers must clear front row before reaching back row.
   */
  useFormations?: boolean;
  /** Wounded ratio for Garrison/BearTrap battles (default 0.6) */
  woundedRatio?: number;
}
...
export interface BattleReport {
  ...
  /** Win rate for attacker across simulations (0-100, Monte Carlo only) */
  attackerWinRate?: number;
  /** Draw rate across simulations (0-100, Monte Carlo only) */
  drawRate?: number;
  /** Total duration of battle in turns */
  totalTurns?: number;
  /** Timestamp when simulation completed */
  timestamp?: number;
  /** Standard deviation of kills across Monte Carlo simulations */
  killsStdDev?: number;
  /** 95% confidence interval for kills [lower, upper] */
  killsConfidenceInterval?: [number, number];
  /** Distribution of remaining troops across Monte Carlo simulations */
...
export interface CombatSideState {
  ...
  /** Active shields absorbing damage */
  activeShields: ActiveShield[];
  /** Active stun/control effects */
  activeStuns: ActiveStun[];
...
/** Default matchup multipliers (rock-paper-scissors type advantage) */
export const DEFAULT_MATCHUP_MULTIPLIERS: Record<TroopType, Record<TroopType, number>> = {
  Infantry: { Infantry: 1.0, Lancer: 1.1, Marksman: 0.9 },
  Lancer:   { Infantry: 0.9, Lancer: 1.0, Marksman: 1.1 },
  Marksman: { Infantry: 1.1, Lancer: 0.9, Marksman: 1.0 },
};
...
export const DEFAULT_BATTLE_CONFIG: BattleConfig = {
  ...
  rngSeed: 1,
  simulations: 200,
  damageFormulaVersion: "v1",
  matchupMultipliers: DEFAULT_MATCHUP_MULTIPLIERS,
  useFormations: false,
  woundedRatio: 0.6,
};

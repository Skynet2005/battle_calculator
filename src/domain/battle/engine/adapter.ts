...
function totalTroops(counts: { infantry?: number; lancer?: number; marksman?: number }): number {
...
export function simulateBattleFromUI(input: UISimulationInput): UISimulationOutput {
  ...
  const attackerTotal = totalTroops(attackerSide.troopCounts ?? {});
  const defenderTotal = totalTroops(defenderSide.troopCounts ?? {});
  if (attackerTotal === 0 && defenderTotal === 0) {
    throw new Error('Cannot simulate battle: both sides have no troops.');
  }

  const maxTurns = input.battleConfig?.maxTurns ?? 30;
  if (maxTurns <= 0) {
    throw new Error('Cannot simulate battle: max turns must be greater than 0.');
  }
...
function mapDamageDebug(action: BattleReport["turns"][number]["actions"][number]): DamageDebug {
  return {
    ...
    // Legacy DamageDebug shape: engine uses single outgoing multiplier for both attack and lethality.

...
import { computeDamage, productFromMagnitudes } from "./damage";
...
function createCasualtyModel(config: BattleConfig) {
  return {
    apply: (kills: number, battleType: BattleType) => {
      if (battleType === "Garrison" || battleType === "BearTrap") {
        const woundedRatio = config.woundedRatio ?? 0.6;
        const wounded = Math.round(kills * woundedRatio);
...
function runMonteCarlo(params: SimulateParams, config: BattleConfig): BattleOutcome {
  const simulations = Math.max(1, config.simulations ?? 200);
  ...
  const defenderInitialTotal =
    params.defender.troops.Infantry + params.defender.troops.Lancer + params.defender.troops.Marksman;

  // Per-simulation tracking for variance and distribution
  ...
  for (let i = 0; i < simulations; i += 1) {
    ...
    const remaining = report.defenderRemaining ?? report.turns[report.turns.length - 1]?.defenderTroops;
    ...
    if (report.attackerRemaining) attackerRemainingPerSim.push({ ...report.attackerRemaining });
    if (report.defenderRemaining) defenderRemainingPerSim.push({ ...report.defenderRemaining });
  ...
  // Compute statistics
  const meanKills = killsPerSim.reduce((s, v) => s + v, 0) / simulations;
  const variance = killsPerSim.reduce((s, v) => s + (v - meanKills) ** 2, 0) / simulations;
  ...
  // Compute remaining troop distribution (min/max/mean)
  const computeTroopDist = (arr: TroopCounts[]) => {
    if (arr.length === 0) {
  ...
  return {
    report: {
      ...
      remainingDistribution: {
        attacker: computeTroopDist(attackerRemainingPerSim),
        defender: computeTroopDist(defenderRemainingPerSim),
      },
    }
  };
}
...
function runDeterministic(params: SimulateParams, config: BattleConfig): BattleReport {
  ...
  const attackerShields: ActiveShield[] = [];
  const defenderShields: ActiveShield[] = [];
  const attackerStuns: ActiveStun[] = [];
  const defenderStuns: ActiveStun[] = [];
  const attackerInitialTroops: TroopCounts = { ...attacker.troops };
  const defenderInitialTroops: TroopCounts = { ...defender.troops };
  ...
  for (let turn = 1; turn <= config.maxTurns; turn += 1) {
    ...
    // Decrement stun durations and remove expired stuns
    expireStuns(attackerStuns);
    expireStuns(defenderStuns);

    // Normal attacks
    for (const attackerType of TROOP_TYPES) {
      if (attackerTroops[attackerType] <= 0) continue;
      if (isStunned(attackerStuns, attackerType)) continue;
      ...
      let killsApplied = Math.round(computation.components.finalKills);
      killsApplied = absorbShields(defenderShields, target, killsApplied);
    ...
    for (const defenderType of TROOP_TYPES) {
      ...
      if (isStunned(defenderStuns, defenderType)) continue;
      ...
      let killsApplied = Math.round(computation.components.finalKills);
      killsApplied = absorbShields(attackerShields, target, killsApplied);
...
function resolveHit(params: {
  ...
  // For skill damage: skill damage is calculated from normal damage as extra damage
  // Following Whiteout Survival model:
  // - Normal Attack Damage% → affects troop normal attacks only
  // - Damage Dealt% → affects all damage sources (troops + pet + hero skill damage)
  // - Skill damage = normal damage base * (1 + SkillDamage%) * (1 + DamageDealt%)
  // - 25% chance to double normal damage, with additional damage being extra (not affected by modifiers)
  if (actionType === "Skill") {
    // Step 1: Calculate normal damage with Normal Attack modifiers + Damage Dealt modifiers
    // Normal Attack modifiers (scope: NormalAttack) only affect normal attacks
    // Damage Dealt modifiers (scope: Any) affect all damage sources
    const normalAttackModifiersList = filterModifiersForAction(
      attackerModifiers.map((m) => m.modifier).filter((m) => m.subject === "outgoing" && m.scope === "NormalAttack"),
      attackerType,
      "NormalAttack"
    );
    const damageDealtModifiersList = filterModifiersForAction(
      attackerModifiers.map((m) => m.modifier).filter((m) => m.subject === "outgoing" && (m.scope === "Any" || !m.scope)),
      attackerType,
      "NormalAttack"
    );
    const normalEnemyOutgoingModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "enemyOutgoing" && m.scope === "NormalAttack"),
      attackerType,
      "NormalAttack"
    );
    const damageDealtEnemyModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "enemyOutgoing" && (m.scope === "Any" || !m.scope)),
      attackerType,
      "NormalAttack"
    );
    const normalIncomingModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "incoming" && m.scope === "NormalAttackReceived"),
      defenderType,
      "NormalAttack"
    );
    const damageDealtIncomingModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "incoming" && (m.scope === "Any" || !m.scope)),
      defenderType,
      "NormalAttack"
    );

    // Combine Normal Attack + Damage Dealt modifiers for normal damage calculation
    const allNormalOutgoingModifiers = [...normalAttackModifiersList, ...damageDealtModifiersList, ...normalEnemyOutgoingModifiersList, ...damageDealtEnemyModifiersList];
    const allNormalIncomingModifiers = [...normalIncomingModifiersList, ...damageDealtIncomingModifiersList];
    const normalOutgoingStack = stackModifiersWithLog(allNormalOutgoingModifiers, config.stackingBehavior);
    const normalIncomingStack = stackModifiersWithLog(allNormalIncomingModifiers, config.stackingBehavior);
    const normalOutgoingMods = normalOutgoingStack.magnitudes;
    const normalIncomingMods = normalIncomingStack.magnitudes;

    const matchup = config.matchupMultipliers?.[attackerType]?.[defenderType] ?? 1;
    const normalDamageComponents = computeDamage(
      {
        attackerType,
        defenderType,
        ...
        actionMultiplier: 1,
        outgoingModifiers: normalOutgoingMods,
        incomingModifiers: normalIncomingMods,
        outgoingDetails: normalOutgoingStack.components,
        incomingDetails: normalIncomingStack.components,
    ...
    const normalDamage = normalDamageComponents.finalKills;

    // Step 2: Calculate skill damage as extra damage
    // Skill damage = normal damage base * (1 + SkillDamage%) * (1 + DamageDealt%)
    // Skill modifiers (scope: Skill) + Damage Dealt modifiers (scope: Any) both apply
    const skillModifiersList = filterModifiersForAction(
      attackerModifiers.map((m) => m.modifier).filter((m) => m.subject === "outgoing" && m.scope === "Skill"),
      attackerType,
      "Skill"
    );
    const skillDamageDealtModifiersList = filterModifiersForAction(
      attackerModifiers.map((m) => m.modifier).filter((m) => m.subject === "outgoing" && (m.scope === "Any" || !m.scope)),
      attackerType,
      "Skill"
    );
    const skillEnemyOutgoingModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "enemyOutgoing" && m.scope === "Skill"),
      attackerType,
      "Skill"
    );
    const skillDamageDealtEnemyModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "enemyOutgoing" && (m.scope === "Any" || !m.scope)),
      attackerType,
      "Skill"
    );
    const skillIncomingModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "incoming" && m.scope === "SkillReceived"),
      defenderType,
      "Skill"
    );
    const skillDamageDealtIncomingModifiersList = filterModifiersForAction(
      defenderModifiers.map((m) => m.modifier).filter((m) => m.subject === "incoming" && (m.scope === "Any" || !m.scope)),
      defenderType,
      "Skill"
    );

    // Combine Skill + Damage Dealt modifiers for skill damage calculation
    const allSkillOutgoingModifiers = [...skillModifiersList, ...skillDamageDealtModifiersList, ...skillEnemyOutgoingModifiersList, ...skillDamageDealtEnemyModifiersList];
    const allSkillIncomingModifiers = [...skillIncomingModifiersList, ...skillDamageDealtIncomingModifiersList];
    const skillOutgoingStack = stackModifiersWithLog(allSkillOutgoingModifiers, config.stackingBehavior);
    const skillIncomingStack = stackModifiersWithLog(allSkillIncomingModifiers, config.stackingBehavior);
    const skillOutgoingMods = skillOutgoingStack.magnitudes;
    const skillIncomingMods = skillIncomingStack.magnitudes;

    // Skill damage is calculated from normal damage base, with skill modifiers + damage dealt modifiers applied
    const skillOutgoingMultiplier = productFromMagnitudes(skillOutgoingMods);
    const skillIncomingMultiplier = productFromMagnitudes(skillIncomingMods);
    const skillDamage = normalDamage * skillOutgoingMultiplier * skillIncomingMultiplier;

    // Step 3: Determine doubling chance from active skills
    // Check active modifiers for their source skills' doubling chances
    // Use the highest doubling chance from any active skill (skills don't stack doubling chances)
    let skillDoubleChance = 0;
    if (skillRuntime && skillRuntime.length > 0) {
      // Find the highest doubling chance from active skills
      for (const skillState of skillRuntime) {
        if (skillState.skill.doubleDamageChance !== undefined) {
          skillDoubleChance = Math.max(skillDoubleChance, skillState.skill.doubleDamageChance);
        }
      }
    }
    // Also check modifiers for skill sources (in case skill isn't in runtime but modifier is active)
    for (const mod of attackerModifiers) {
      // Try to find the skill definition from the modifier source
      // Modifier source format: "HeroName: SkillName" or "skillId"
      const sourceParts = mod.modifier.source?.split(':') || [];
      if (sourceParts.length >= 2 && skillRuntime) {
        const skillName = sourceParts[1].trim();
        for (const skillState of skillRuntime) {
          if (skillState.skill.name === skillName && skillState.skill.doubleDamageChance !== undefined) {
            skillDoubleChance = Math.max(skillDoubleChance, skillState.skill.doubleDamageChance);
          }
        }
      }
    ...
    // If a specific skillId is provided, try to find that skill's doubling chance
    if (skillId && skillRuntime) {
      for (const skillState of skillRuntime) {
        if (skillState.skill.id === skillId && skillState.skill.doubleDamageChance !== undefined) {
          skillDoubleChance = Math.max(skillDoubleChance, skillState.skill.doubleDamageChance);
        }
      }
    }
    
    // Default to 0.25 (25%) if no skill-specific chance found and this is a skill action
    // This maintains backward compatibility while allowing skill-specific overrides
    const doubleChance = skillDoubleChance > 0 ? skillDoubleChance : 0.25;
    
    // Apply chance to double normal damage
    // The additional damage from doubling is EXTRA damage (not affected by any modifiers)
    let extraDamage = 0;
    
    if (config.randomMode === "expectedValue") {
      // In expected value mode, scale by probability
      // X% chance of doubling means X% * normalDamage extra damage
      extraDamage = normalDamage * doubleChance;
    } else {
      // In Monte Carlo mode, roll for doubling
      const roll = rng();
      if (roll <= doubleChance) {
        // Double the normal damage - the additional normalDamage is extra damage (raw, no modifiers)
        extraDamage = normalDamage;
      }
    }
    
    // Total damage = normal damage + skill damage (extra) + extra damage from doubling
    // Following: TroopNormalDamage' + ExtraDamage' where ExtraDamage' includes skill damage
    const totalDamage = normalDamage + skillDamage + extraDamage;
    const finalKills = Math.min(Math.max(0, totalDamage), defenderCount);

    // Combine modifier components for logging
    const combinedOutgoingComponents = [...normalOutgoingStack.components, ...skillOutgoingStack.components];
    const combinedIncomingComponents = [...normalIncomingStack.components, ...skillIncomingStack.components];

    return {
      components: {
        ...normalDamageComponents,
        finalKills,
        rawFinal: totalDamage,
        baseKills: normalDamageComponents.baseKills + (skillDamage > 0 ? normalDamageComponents.baseKills : 0) + (extraDamage > 0 ? normalDamageComponents.baseKills : 0),
        outgoingMultiplier: normalDamageComponents.outgoingMultiplier * (skillDamage > 0 ? skillOutgoingMultiplier : 1),
        incomingMultiplier: normalDamageComponents.incomingMultiplier * (skillDamage > 0 ? skillIncomingMultiplier : 1)
      },
      attackerDetail: attackerStatBuff.detail,
      defenderDetail: defenderStatBuff.detail,
      outgoingComponents: combinedOutgoingComponents,
      incomingComponents: combinedIncomingComponents
    };
  }

  // Normal attack path (unchanged)
...
function applySkillTrigger(
  ...
  effects.forEach((effect) => {
    ...
    if (effect.type === "Shield" && shields && effect.flatDamage) {
      shields.push({
        source: effect.id,
        remaining: effect.flatDamage,
        appliesTo: effect.target,
      });
    }
    if (effect.type === "Heal" && initialTroops && effect.flatDamage) {
      applyHealing(currentTroops, initialTroops, effect.target, effect.flatDamage);
    }
  });
...
/**
 * Absorb damage through active shields. Returns the remaining kills after absorption.
 */
function absorbShields(shields: ActiveShield[], targetType: TroopType, kills: number): number {
  let remaining = kills;
  for (let i = shields.length - 1; i >= 0; i--) {
    const shield = shields[i];
    if (shield.appliesTo !== "All" && shield.appliesTo !== targetType) continue;
    if (remaining <= 0) break;
    const absorbed = Math.min(remaining, shield.remaining);
...
/**
 * Check if a specific troop type is stunned.
 */
function isStunned(stuns: ActiveStun[], troopType: TroopType): boolean {
  return stuns.some(
    (s) => s.remaining > 0 && (s.target === "All" || s.target === troopType)
  );
}

/**
 * Decrement stun durations and remove expired ones.
 */
function expireStuns(stuns: ActiveStun[]): void {
  for (let i = stuns.length - 1; i >= 0; i--) {
    stuns[i].remaining -= 1;
    if (stuns[i].remaining <= 0) {
      stuns.splice(i, 1);
    }
  }
}
...
/**
 * Apply healing: restore troop count up to initial count.
 */
function applyHealing(
  ...
  const apply = (type: TroopType) => {
    const max = initialTroops[type];
    troops[type] = Math.min(max, troops[type] + Math.round(healAmount));
  };
  if (target === "All") {
    TROOP_TYPES.forEach(apply);
  } else {
    apply(target as TroopType);
  }
}

...
import type {
  ...
  SkillLevel,
  SkillLevelKey
} from "../battle/data/heroes/hero_types";
import type {
  BonusTarget,
  DamageModifier,
  DamageModifierScope,
...
function determineTrigger(skill: LevelSkill | null): SkillTrigger {
  ...
  if (skill.trigger_every_n_turns !== undefined ||
    (skill as any).trigger_every_n_strikes !== undefined) {
...
function getPeriodicInterval(skill: LevelSkill | null): number | undefined {
  ...
  if (skill.trigger_every_n_turns !== undefined) {
    const val = typeof skill.trigger_every_n_turns === 'number'
      ? skill.trigger_every_n_turns
      : typeof skill.trigger_every_n_turns === 'string'
        ? parseInt(skill.trigger_every_n_turns, 10) || 0
        : extractSkillValue(skill.trigger_every_n_turns, 5);
    return val;
  }
  // Handle "every N strikes" (treat as "every N turns" for now)
  if ((skill as any).trigger_every_n_strikes !== undefined) {
    const val = typeof (skill as any).trigger_every_n_strikes === 'number'
      ? (skill as any).trigger_every_n_strikes
      : typeof (skill as any).trigger_every_n_strikes === 'string'
        ? parseInt((skill as any).trigger_every_n_strikes, 10) || 0
        : extractSkillValue((skill as any).trigger_every_n_strikes, 5);
    return val;
  }
  if (skill.trigger_after_every_n_turns !== undefined) {
    const val = typeof skill.trigger_after_every_n_turns === 'number'
      ? skill.trigger_after_every_n_turns
      : typeof skill.trigger_after_every_n_turns === 'string'
        ? parseInt(skill.trigger_after_every_n_turns, 10) || 0
        : extractSkillValue(skill.trigger_after_every_n_turns, 5);
    return val;
  }
  return undefined;
}
...
export function convertJoinersToSkillDefinitions(
  ...
  getHeroExpeditionSkills: (hero: any) => Array<{ name: string; data: LevelSkill }>
): SkillDefinition[] {
...
export function convertLeadersToSkillDefinitions(
  ...
  const leaderTypes = ['infantry', 'lancer', 'marksman'] as const;

  leaderTypes.forEach((type) => {
...
// ---------------------------------------------------------------------------
import type { TroopType } from "../battle";
import { TROOP_SKILLS } from "../battle/data/troops/troop_skills";
...
export function convertTroopSkillsToSkillDefinitions(troopType: TroopType): SkillDefinition[] {
  ...
  return skills.map((skill, idx) => {
    ...
    if (effects.length === 0) {
      const skillEffects = (skill as any).skill_effects as any[] | undefined;
      if (Array.isArray(skillEffects)) {
        skillEffects.forEach((se, sidx) => {
          ...
          const chance = se?.effect_is_chance
            ? Math.max(...Object.values(se.effect_probabilities || { "1": 0 }).map((n: any) => Number(n) / 100 || 0))
            : 1;

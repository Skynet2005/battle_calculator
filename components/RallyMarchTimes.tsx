          ...
          <div className="grid gap-4">
            {rallyLeads.map((lead, index) => (
              <div key={lead.id} className="flex items-center gap-4 p-4 border border-slate-700 rounded-lg bg-slate-800/50">
                <div className="flex-shrink-0 w-8 h-8 bg-emerald-600 rounded-full flex items-center justify-center text-sm font-semibold">
                  {index + 1}
                </div>
                ...
                <button
                  ...
                  className="flex-shrink-0 p-2 text-red-400 hover:text-red-300 hover:bg-red-900/20 disabled:text-slate-500 disabled:hover:bg-transparent rounded transition-colors"

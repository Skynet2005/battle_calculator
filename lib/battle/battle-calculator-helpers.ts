...
export function buildSideBaseStats(
  ...
  return TROOP_TYPE_LIST.reduce((acc, troop) => {
    ...
    const finalStats = calculateFinalStats(basicWithHero, normalizedAdditive, normalizedMultiplicative, troop);
...
export function normalizeTroopMix(mix: TroopMixConfig): TroopMixConfig {
  ...
  const totalTroops = Math.max(0, base.totalTroops || 0);
  const ratios = {
    infantry: Math.max(0, base.infantryRatio ?? DEFAULT_TROOP_MIX.infantryRatio),
    lancer: Math.max(0, base.lancerRatio ?? DEFAULT_TROOP_MIX.lancerRatio),
    marksman: Math.max(0, base.marksmanRatio ?? DEFAULT_TROOP_MIX.marksmanRatio)
  };
  const ratioSum = ratios.infantry + ratios.lancer + ratios.marksman;
  const normalizedRatio =
    ratioSum === 0
      ...
      : {
        infantry: (ratios.infantry / ratioSum) * 100,
        lancer: (ratios.lancer / ratioSum) * 100,
        marksman: (ratios.marksman / ratioSum) * 100
      };

  return {
    totalTroops,
    infantryRatio: Number(normalizedRatio.infantry.toFixed(2)),
    lancerRatio: Number(normalizedRatio.lancer.toFixed(2)),
    marksmanRatio: Number(normalizedRatio.marksman.toFixed(2))
  };
}
